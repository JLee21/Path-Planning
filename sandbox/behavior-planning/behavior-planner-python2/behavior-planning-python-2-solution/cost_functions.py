...
def calculate_cost(vehicle, trajectory, predictions, verbose=False):
	...
	for cf in [
		...
		if DEBUG or verbose:
			cprint("{:30} for lane {} has cost {:>10.2f}" \
				.format(cf.__name__, trajectory[-1].lane, new_cost), 'yellow')
		cost += new_cost
	return cost
...
def get_helper_data(vehicle,trajectory,predictions):
	"""
	:trajectory:
		trajectory for state LCR
		Snapshot(lane=0, s=221, v=9, a=2, state='LCL')
		Snapshot(lane=1, s=230, v=4, a=-5, state='LCR')
		Snapshot(lane=1, s=230, v=4, a=-5, state='LCR')
		Snapshot(lane=1, s=230, v=5, a=-4, state='LCR')
		Snapshot(lane=1, s=230, v=10, a=1, state='LCR')
		Snapshot(lane=1, s=230, v=10, a=1, state='LCR')
	"""
	t = trajectory
